...
func (repo *Repository) GetAll(ctx context.Context, limit, offset uint32) ([]*Basket, error) {
	var baskets []*Basket
	...
	query := `SELECT * FROM basket ORDER BY created_at DESC LIMIT $1 OFFSET $2;`
...
func (repo *Repository) GetById(ctx context.Context, id types.UuidType) (*Basket, error) {
	basket := &Basket{}
...
func (repo *Repository) HasById(ctx context.Context, id types.UuidType) (bool, error) {
	...
	query := `SELECT EXISTS(SELECT 1 FROM basket WHERE id = $1);`
...
func (repo *Repository) Find(ctx context.Context, dto *FindDTO, limit, offset uint32) ([]*Basket, error) {
...
func fillQueryForFind(queryBuilder *db.QueryBuilder, dto *FindDTO) *db.QueryBuilder {
	...
	if dto.UserId != nil { // Nullable
		queryBuilder = queryBuilder.
			...
			SetParameter(":user_id", *dto.UserId)
	}

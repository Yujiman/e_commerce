...
func (q *QueryBuilder) buildQuery() string {
	...
	if len(q.query) > 0 {
		if strings.Contains(q.query[0], "OR") {
			q.query[0] = strings.Replace(q.query[0], "OR", "WHERE", 1)
		} else if strings.Contains(q.query[0], "AND") {
			q.query[0] = strings.Replace(q.query[0], "AND", "WHERE", 1)
		}
	}

...
import (
	...
	"github.com/Yujiman/e_commerce/auth/aggregatorDomain/proto/domain"
...
func GetAll(req *domain.GetAllRequest) (*domain.Domains, error) {
	var addr = config.GetServicesParams().Domain
	clientConn, err := service.GetGrpcClientConnection(addr)
	defer func() {
		if clientConn != nil {
	...
	if ctx.Err() == context.DeadlineExceeded {
		return nil, status.Error(codes.Code(503), "Client to Domain:GetAll service timeout exceeded.")
	}
	if err != nil {
		return nil, err
	}
...
func Find(req *domain.FindRequest) (*domain.Domains, error) {
	var addr = config.GetServicesParams().Domain
	clientConn, err := service.GetGrpcClientConnection(addr)
	defer func() {
		if clientConn != nil {
	...
	if ctx.Err() == context.DeadlineExceeded {
		return nil, status.Error(codes.Code(503), "Client to Domain:Find service timeout exceeded.")
	}
	if err != nil {
		return nil, err
	}

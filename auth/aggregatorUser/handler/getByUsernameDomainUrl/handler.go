package getByUsernameDomainUrl
...
func Handle(req *pb.GetByUsernameDomainUrlRequest) (*pb.User, error) {
	...
	userDomains, err := getUserDomainProto(userResp.Domains, req.DomainUrl)
	if err != nil {
		return nil, err
	}
...
func getUserDomainProto(userDomainsResp []*pbOauthUser.DomainDetail, reqDomainId string) ([]*pb.UserDomain, error) {
	domainResp, err := domain.GetByUrl(&pbDomain.GetByUrlRequest{Url: reqDomainId})
	if err != nil {
		return nil, err
	}

...
func Handle(req *pb.AddWithIdRequest) (*pb.Empty, error) {
	...
	hasById, err := user.HasById(ctx, id)
	if err != nil {
		return nil, err
	}
	if hasById {
		return nil, status.Error(codes.Code(409), "User with this id already exists.")
	}
...
func validateRequest(req *pb.AddWithIdRequest) error {
	if err := utils.CheckUuid(req.Id); err != nil {
		return status.Error(codes.Code(400), "User id must be uuid type.")
	}
	...
	if req.RoleId == "" {
		return status.Error(codes.Code(400), "role_id can't be empty.")
	}
	return nil
}

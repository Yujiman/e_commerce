...
func NewTransaction(ctx context.Context, options *sql.TxOptions) (*Transaction, error) {
	...
	if err != nil {
		...
		return nil, status.Error(codes.Code(500), "Transaction:NewTransaction; Error="+err.Error())
	}
...
func (tr *Transaction) PersistNamedCtx(ctx context.Context, query string, entity interface{}) error {
	...
	if err != nil {
		return status.Error(codes.Code(500), "Transaction:PersistNamedCtx query="+query+"; Error="+err.Error())
	}
	return nil
}

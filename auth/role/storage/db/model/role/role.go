...
type Role struct {
	...
	DomainId  types.UuidType   `db:"domain_id"`
...
func (r Role) isRequiredEmpty() bool {
	return r.Id.String() == "" || r.CreatedAt.IsZero() || r.UpdatedAt.IsZero() ||
		r.Name.Name() == "" || r.DomainId.String() == "" || r.Scopes.String() == ""
}
...
func (r *Role) SaveNew(tr *db.Transaction, ctx context.Context) (err error) {
	...
	has, err := HasByNameDomain(ctx, r.Name, r.DomainId)
	if err != nil {
		return err
	}
	if has {
		return status.Error(codes.Code(409), "Domain already has role with this name.")
	}
	query := `INSERT INTO role(id, created_at, updated_at, name, domain_id, scopes)
				VALUES (:id, :created_at, :updated_at, :name, :domain_id, :scopes);`
...
func (r *Role) ChangeName(tr *db.Transaction, ctx context.Context, nameType types.NameType) (err error) {
	...
	has, err := HasByNameDomain(ctx, nameType, r.DomainId)
	if err != nil {
		return err
	}
	if has {
		return status.Error(codes.Code(409), "Domain already has role with this name."+nameType.Name())
	}

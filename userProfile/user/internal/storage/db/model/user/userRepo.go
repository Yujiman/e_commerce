...
func (repo *Repository) GetAll(ctx context.Context, limit, offset uint32) ([]*User, error) {
	var users []*User
...
func (repo *Repository) GetCountAllForFind(ctx context.Context, dto *FindDTO) (uint32, error) {
	...
	// Prepare QUERY
	queryBuilder := db.NewQueryBuilder("user").
		Select("COUNT(id)")
...
func (repo *Repository) Find(ctx context.Context, dto *FindDTO, limit, offset uint32) ([]*User, error) {
	...
	err := repo.DbCon.SelectContext(ctx, &users, query, queryBuilder.GetParams()...)
	switch err {
	case nil, sql.ErrNoRows:
		return users, nil
	default:
